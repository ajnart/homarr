...
const SwaggerApiPage = ({ authenticationToken }: { authenticationToken: string }) => {
  const [accessTokenRevealed, { toggle: toggleAccessTokenReveal, close: hideAccessToken }] =
    useDisclosure(false);
  ...
  return (
    <ManageLayout>
      ...
      <Text mb={'xl'}>
        Advanced users can use the API to interface with Homarr. The documentation is completely
        local, interactive and complies with the Open API standard. Any compatible client can import
        for easy usage.
      </Text>

      <Group>
        <Button
          onClick={toggleAccessTokenReveal}
          leftIcon={<IconLockAccess size={'1rem'} />}
          variant={'light'}
        >
