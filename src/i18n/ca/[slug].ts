...
function Put(req: NextApiRequest, res: NextApiResponse) {
  ...
  fs.writeFileSync(path.join('data/configs', `${slug}.json`), JSON.stringify(body, null, 2), 'utf8');
...
function Get(req: NextApiRequest, res: NextApiResponse) {
  ...
  const files = fs.readdirSync('data/configs'); // Strip the .json extension from the file name

  const configs = files.map(file => file.replace('.json', '')); // If the target is not in the list of files, return an error
